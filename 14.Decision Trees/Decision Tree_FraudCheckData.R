...
model1 <- ctree(as.factor(traintax$tax_cat)~City.Population+Work.Experience,data = traintax)
pred1<-predict(model1,testtax) 
table(pred1,testtax$tax_cat)
mean(pred1==testtax$tax_cat)               #accuracy= 79.33%

#model2 using c50 package 
model2 <- C5.0(as.factor(tax_cat)~.,data = traintax,trails=100)
...
for(i in 1:100){
  ...
  trains <- data1[splits,]
  tests <- data1[-splits,]
  model4 <- rpart(trains[,-6],as.factor(trains$tax_cat))
