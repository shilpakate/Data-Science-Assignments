...
forest <- randomForest(as.factor(tax_cat)~.,data = train_f,importance=TRUE,mtry=2)
forest
...
for(i in 2:10){
  ...
  train_d <- frauddata[d,]
  test_d <- frauddata[-d,]
  model_d <- randomForest(as.factor(tax_cat)~.,data = train_d,mtry=i)
...
finalmodel <- randomForest(as.factor(tax_cat)~.,data = train_f,mtry=2)
finalmodel
